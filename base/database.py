...
class Database:
    ...
    def insert_record_shipment(self, ship) -> int:
        ...
        with self.connection.cursor() as cursor:
            cursor.execute(query.select_client, (ship.client['fsrar_id'],))
            try:
            ...
            if fsrar_id == None:
                cursor.execute(query.insert_client, (ship.client['name'], ship.client['fsrar_id'], 
                                     ship.client['INN'], ship.client['KPP'], 
                                     ship.client['address']))
...
class Async_database:
    def __init__(self, user, loop=None) -> None:
        ...
        try:
            self.pool: asyncpg = loop.run_until_complete(asyncpg.create_pool(
                                    host = self.config_database['host'],
                                    user = self.config_database['user'],
                                    password = self.config_database['password'],
                                    database = self.config_database['db_name']))
        except Exception as _ex:
            logger.error(f'BOT - POSTGRES ERROR: {_ex}')
