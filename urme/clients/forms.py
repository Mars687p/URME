...
from users.services import COLUMNS_CLIENTS
...
class Pick_display_cl_clients(forms.Form):
    columns = forms.MultipleChoiceField(
                    ...
                    choices=[i for i in COLUMNS_CLIENTS.items() if i[0] != 'fsrar_id'],
                    required=False)


class Select_date(forms.Form):
    date_select = forms.ChoiceField(widget=forms.RadioSelect(
                                            attrs={'onchange': 'listen_select(value)'}),
                                    choices=(('month', 'Месяц'),
                                             ('year', 'Год')),
                                    required=False
                                    )
